...
private:
    ...
    gnnt::mnist_dataset<value_type> dataset = gnnt::transform<value_type>(gnnt::filter(
            gnnt::mnist_serializer::read("../data/mnist"),
            [](auto lbl) { return lbl < 4; }
    ), [=](auto begin, auto end, auto dst) {
        gnnt::normalize(begin, end, dst, 0, 255);
    });
    neural_network nn{};
